module AutoCompleteMacrosHelper
  ...
  def plugin_auto_complete_field(field_id, options = {})
    ...
    js_options[:paramName]  = "'value'" # have string being sent at a set location so answer method knows where to find it
  ...
  def text_field_with_auto_complete(object, method, tag_options = {}, completion_options = {})
    object_short = object.to_s.demodulize.downcase
    field_id = tag_options[:id] || "#{object_short}_#{method}" # added by ChrisG - allow user to provide own id for auto-complete fields
    (completion_options[:skip_style] ? "" : auto_complete_stylesheet) +
    text_field(object_short, method, tag_options) +
    content_tag("div", "", :id => "#{field_id}_auto_complete", :class => "auto_complete") +
    plugin_auto_complete_field(field_id, { :url => { :action => "auto_complete_for_#{object_short}_#{method}" } }.update(completion_options))
  end
