class Lexer
  ...
  def pop(token_class)
    ...
    if token.class != token_class
        raise(ParseError,"expected #{token_class.to_s} but received #{token.class.to_s} at #{@input[0..10]}...", caller)
    else
        return token
    end
  end
   ...
   def read_next_token(token_class)
      ...
      if match(token_class)
        ...
        Tokens.list.each {|t|
         ...
         if @input != ''
             raise( ParseError, "Lex Error, unknown token at #{@input[0..10]}...", caller)
         else
            return nil
      end
    end
