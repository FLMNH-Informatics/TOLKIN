...
Module('Taxa', function () {
  JooseClass('NewPage', {
    ...
    has: {
      ...
      width: { is: 'ro', init: 780 },
      title: { is: 'ro', init: 'Taxon : New' },
...
//          })
      ...
      htmlLoader: { is: 'ro', init: function () { return $HtmlLoader({
        pathname: 'new_project_taxon_path'
      }, this) } }
    },
    methods: {
//      onSubmit: function (event) {
//        if(event.element().hasClassName('new_citation')){
//          var fields = event.element().serialize({
//            hash: true,
//            submit: false
//          });
...
//          this.poll({
//            on: function () {
//              return $('upload_frame').$('citation_status')
//            },
//            run: function() {
//              this._appendNewCitation(fields);
...
//          $('div_taxon_'+this.record('taxon').id()+'_citations').insert({
//            bottom: evaledTemplate
//          });
...
//          }
//          var atts = this.record('taxon').attributes();
      ...
      onClick: function (event) {
        ...
        Event.delegate({
          '.saveButton.active': function (event) {
            event.stop()
            this.notifier().working('Creating taxon... ')
            $(this.frame().id()).down('form').request({
              requestHeaders: { Accept: 'application/json' },
              onSuccess: function (transport) {
                me.notifier().success('Taxon created.')
                new Taxon({ context: me.context() }).fire('create')
                var rpath = 'project_taxon_path'
                if(window.location.href.indexOf('tree_view') === -1 ){
                  me.frame().loadPage(rpath, { id: transport.responseJSON.id })
                }
                close ? me.frame().close() : me.frame().back()
              }
            })
//            event.element().request({
//              onSucess: function () {
//                me.notifier().success('OTU successfully created.')
//                var taxon = new Taxon({ context: me.context() })
//                taxon.fire('create', {memo: { record: otu } })
//              },
//              onFailure: function () {
//                me.notifier().error('Problem encountered: Taxon could not be saved')
//                }
//              })
          },
