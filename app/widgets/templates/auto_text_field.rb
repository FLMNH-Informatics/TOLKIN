class Templates::AutoTextField < Widget
  ...
  def name
    @value_method == [*attribute_path].first.singularize ?
      "#{@object_name}[#{[*attribute_path].first.singularize}]"
        : "#{@object_name}[#{[*attribute_path].first.singularize}_#{@value_method}]"

  end

  def text_field_name
    @text_method == [*attribute_path].first.singularize ?
      "#{@object_name}[#{[*attribute_path].first.singularize}]"
        : "#{@object_name}[#{[*attribute_path].first.singularize}_#{@text_method}]"
   # "#{@object_name}[#{[*attribute_path].first.singularize}_#{@text_method}]"
  end

  def text_field_input_id
     "#{@object_name}_#{[*attribute_path].first.singularize}_#{@text_method}_auto_input"
  end
  ...
  def render_to_string
    ...
    when 'edit'
      render partial: 'widgets/auto_text_field'
    end
  end
end
