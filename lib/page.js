...
//    lastClicked:   { is: 'rw', lazy: true }
  ...
  after: {
    initialize: function () {
      ...
      if (window.location.pathname.split('/').last() == "new_upload" && this.toString().split('.').last() == "Blank"){
        //another hacky thing, should create a page for this, but don't have time to figure out how to integrate w/ routes/controller_actions
        this.frame().viewport().loadPage('new_upload_project_collections_path')
      }
      this.handlers().push(
