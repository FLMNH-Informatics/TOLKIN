...
//                acc[':'+splitPath[i-1].singularize()+'_id'] :
  ...
  forControllerAction: function (controller, action) {
    ...
    if (controller == "molecular/matrices/submatrices"){
      controller = "molecular/matrix/submatrices"
    }
    if (action=='update')action='show';//update and show should have same route, just the method for update is put instead of get
