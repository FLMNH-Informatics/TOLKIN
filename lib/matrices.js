...
function sizeTableContainers(bodyExpanderWidth) {
  ...
  var userPanelWidth = $('viewport').down('.user_panel') ? $('viewport').down('.user_panel').getDimensions().width : 0;
...
function setTableCellWidth(otu_id, chr_id) {
  var width = ($('ch_' + chr_id).getWidth()) + 'px'
  $("c_" + otu_id + "_" + chr_id).setStyle({ width: width });
}
